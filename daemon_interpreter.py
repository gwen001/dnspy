...
#
# run the interpreter of the output of the resolver
#
def runInterpreter( domain ):

    domain_dir = domains_dir + '/' + domain

    cmd = dnspy_dir + '/interpreter.py -f ' + dnspy_dir + '/fingerprints.json -s ' + domain_dir + '/resolve.txt | tee -a ' + domain_dir + '/takeovers | tee -a ' + takeovers_file
    print( cmd )
...
def runDomain( domain ):
    ...
    if not os.path.isfile(resolve_file):
        sys.stdout.write( "%s[-] %s something went wrong, resolve file doesn't exist%s\n" % (fg('red'),domain,attr(0)) )
        return

    if not runInterpreter( domain ):
        return
...
queue_file = dnspy_dir + '/queue_interpreter'
takeovers_file = dnspy_dir + '/takeovers'
